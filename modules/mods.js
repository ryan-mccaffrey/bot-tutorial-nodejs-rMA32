...
exports.getCmdListDescription = function () {
  return [
    ...
    {cmd: "/mod remove 'name'", desc: "Owner command to remove mods", owner: true}
  ];
}
...
// In progress function for removing mods
function rmvModCmd(request, owner, callback) {
  var regex = /^\/mod remove (.+?)$/;

  if (regex.test(request.text)) {
    if (request.user_id != owner.id) {
      callback(true, "You wish you could demote mods", []);
      return "You wish you could demote mods.";
    }
